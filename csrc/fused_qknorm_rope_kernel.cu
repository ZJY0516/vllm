...
__forceinline__ __device__ int pick_mrope_axis(int half_dim,
                                               int mrope_section_t,
                                               int mrope_section_h,
                                               int mrope_section_w,
  ...
  if (mrope_interleaved) {
    bool const h_mask =
        ((half_dim % 3) == 1) && (half_dim <= 3 * mrope_section_h);
    bool const w_mask =
        ((half_dim % 3) == 2) && (half_dim <= 3 * mrope_section_w);
    if (h_mask) return 1;
    if (w_mask) return 2;
    return 0;
  }
  ...
  int const h_end = t_end + mrope_section_h;
  if (half_dim < t_end) return 0;
  if (half_dim < h_end) return 1;
  return 2;
}
...
__global__ void fusedQKNormRopeKernel(
    ...
    int const position_stride,       // Number of columns in position_ids
    bool const use_mrope,            // Whether position_ids is [3, num_tokens]
    int const mrope_section_t,       // mRoPE section for temporal axis
    int const mrope_section_h,       // mRoPE section for height axis
    int const mrope_section_w,       // mRoPE section for width axis
    bool const mrope_interleaved,    // mRoPE dim layout flag
...
#pragma unroll
    ...
    int64_t pos_id_t = pos_id;
    int64_t pos_id_h = pos_id;
    int64_t pos_id_w = pos_id;
    if (use_mrope) {
      pos_id_t = position_ids[tokenIdx];
      pos_id_h = position_ids[position_stride + tokenIdx];
      pos_id_w = position_ids[2 * position_stride + tokenIdx];
    }
...
#pragma unroll
        for (int i = 0; i < numElemsPerThread / 2; ++i) {
          ...
          T_cache const* cos_ptr_dim = cos_ptr;
          T_cache const* sin_ptr_dim = sin_ptr;
          if (use_mrope) {
            int const axis =
                pick_mrope_axis(half_dim, mrope_section_t, mrope_section_h,
                                mrope_section_w, mrope_interleaved);
            int64_t const pos_for_dim =
                axis == 0 ? pos_id_t : (axis == 1 ? pos_id_h : pos_id_w);
            T_cache const* cache_ptr_dim =
                cos_sin_cache + pos_for_dim * rotary_dim;
            cos_ptr_dim = cache_ptr_dim;
            sin_ptr_dim = cache_ptr_dim + embed_dim;
          }
          float const cos_val =
              CacheConverter::convert(VLLM_LDG(cos_ptr_dim + half_dim));
          float const sin_val =
              CacheConverter::convert(VLLM_LDG(sin_ptr_dim + half_dim));
...
__global__ void fusedQKNormRopeKernelNTokenHeads(
    ...
    int64_t const* position_ids, int const position_stride,
    bool const use_mrope, int const mrope_section_t, int const mrope_section_h,
  ...
  if constexpr ((std::is_same_v<scalar_t_in, c10::BFloat16>) ||
    ...
    int const cos_sin_bytes = use_mrope ? 0
                                        : warpsPerBlock * rotary_dim *
                                              static_cast<int>(sizeof(T_cache));
    ...
    if (!use_mrope) {
      ...
      // wait<1>: allow at most 1 pending group (group 1) → group 0 (QKV) is
      // done.
      rope_cp_async_wait<1>();
    } else {
      // Only group 0 (QKV) was issued.
...
#pragma unroll
      ...
      if (!use_mrope && k == 0) rope_cp_async_wait<0>();
...
#pragma unroll
          for (int i = 0; i < numElemsPerThread / 2; ++i) {
            ...
            if (use_mrope) {
              ...
              int64_t const pos_for_dim =
                  axis == 0 ? pos_id_t : (axis == 1 ? pos_id_h : pos_id_w);
              T_cache const* const cache_ptr_dim =
                  cos_sin_cache + pos_for_dim * rotary_dim;
              cos_val =
                  CacheConverter::convert(VLLM_LDG(cache_ptr_dim + half_dim));
              sin_val = CacheConverter::convert(
                  VLLM_LDG(cache_ptr_dim + embed_dim + half_dim));
            } else {
              cos_val = CacheConverter::convert(cos_smem[half_dim]);
              sin_val = CacheConverter::convert(sin_smem[half_dim]);
            }
            elements[idx0] = val0 * cos_val - val1 * sin_val;
            elements[idx1] = val0 * sin_val + val1 * cos_val;
          }
...
void launchFusedQKNormRope(void* qkv, int const num_tokens,
                           ...
                           int const position_stride, bool const use_mrope,
      ...
      DISPATCH_INTERLEAVE(interleave, INTERLEAVE, {
        fusedQKNormRopeKernel<scalar_t_in, scalar_t_cache, 64, INTERLEAVE>
            <<<gridDim, blockDim, 0, stream>>>(
                ...
                k_weight, cos_sin_cache, position_ids, position_stride,
                use_mrope, mrope_section_t, mrope_section_h, mrope_section_w,
                mrope_interleaved, num_tokens, rotary_dim);
      });
      break;
    case 128:
      DISPATCH_INTERLEAVE(interleave, INTERLEAVE, {
...
void launchFusedQKNormRopeNTokenHeads(
  ...
  if (token_heads_per_warp == 1) {
    launchFusedQKNormRope<scalar_t_in, scalar_t_cache>(
        ...
        position_ids, position_stride, use_mrope, mrope_section_t,
        mrope_section_h, mrope_section_w, mrope_interleaved, stream);
    return;
  }
        ...
        DISPATCH_INTERLEAVE(interleave, INTERLEAVE, {                          \
          fusedQKNormRopeKernelNTokenHeads<scalar_t_in, scalar_t_cache, 64,    \
              ...
              <<<gridDim, blockDim, smem_bytes, stream>>>(                     \
                  qkv, num_heads_q, num_heads_k, num_heads_v, eps, q_weight,   \
                  k_weight, cos_sin_cache, position_ids, position_stride,      \
                  use_mrope, mrope_section_t, mrope_section_h,                 \
                  mrope_section_w, mrope_interleaved, num_tokens, rotary_dim); \
        });                                                                    \
        break;                                                                 \
      case 128:                                                                \
        DISPATCH_INTERLEAVE(interleave, INTERLEAVE, {                          \
...
void fused_qk_norm_rope(
    ...
    torch::Tensor& position_ids,   // Position IDs for RoPE
    int64_t mrope_section_t,       // mRoPE section for temporal axis
    int64_t mrope_section_h,       // mRoPE section for height axis
    int64_t mrope_section_w,       // mRoPE section for width axis
    bool mrope_interleaved,        // mRoPE dim layout flag
  ...
  TORCH_CHECK(qkv.dim() == 2,
              ...
              "(num_heads_q+num_heads_k+num_heads_v)*head_dim]");
  TORCH_CHECK(position_ids.dim() == 1 || position_ids.dim() == 2,
              "Position IDs must be 1D ([num_tokens]) or "
              "2D ([3, num_tokens])");
  ...
  bool const use_mrope = position_ids.dim() == 2;
  int64_t mrope_section_t_local = mrope_section_t;
  int64_t mrope_section_h_local = mrope_section_h;
  int64_t mrope_section_w_local = mrope_section_w;
  int64_t position_stride = 0;
  if (use_mrope) {
    TORCH_CHECK(position_ids.size(0) == 3,
                "mRoPE position_ids must be shaped [3, num_tokens]");
    TORCH_CHECK(position_ids.size(1) == num_tokens,
                "Number of tokens in position_ids must match QKV");
    position_stride = position_ids.size(1);

    TORCH_CHECK(mrope_section_t_local >= 0 && mrope_section_h_local >= 0 &&
                    mrope_section_w_local >= 0,
                "mRoPE sections must be non-negative");
    // Backward compatibility for legacy callers (e.g., old compile patterns)
    // that pass 2D position_ids but do not provide explicit mRoPE sections.
    if (mrope_section_t_local == 0 && mrope_section_h_local == 0 &&
        mrope_section_w_local == 0) {
      mrope_section_t_local = cos_sin_cache.size(1) / 2;
    } else {
      TORCH_CHECK(mrope_section_t_local + mrope_section_h_local +
                          mrope_section_w_local ==
                      cos_sin_cache.size(1) / 2,
                  "mRoPE section sizes must sum to rotary_dim/2");
    }
  } else {
    ...
    VLLM_DISPATCH_FLOATING_TYPES(
        cos_sin_cache.scalar_type(), "fused_qk_norm_rope_kernel", [&] {
          ...
          tensorrt_llm::kernels::launchFusedQKNormRopeNTokenHeads<
              ...
              static_cast<int>(position_stride), use_mrope,
              static_cast<int>(mrope_section_t_local),
              static_cast<int>(mrope_section_h_local),
              static_cast<int>(mrope_section_w_local), mrope_interleaved,
