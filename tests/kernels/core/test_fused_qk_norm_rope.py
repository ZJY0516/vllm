...
@pytest.mark.parametrize("forced_token_heads_per_warp", [2, 4])
@torch.inference_mode()
def test_fused_qk_norm_rope_matches_mrope_reference(
    default_vllm_config,
    device: str,
    dtype: torch.dtype,
    eps: float,
    ...
    qkv_base = torch.randn(num_tokens, total_dim, dtype=dtype, device=device)
    qkv_fused = qkv_base.clone()
    positions = torch.randint(0, 1024, (3, num_tokens), dtype=torch.long, device=device)
    ...
    if dtype == torch.float16:
        ...
        # MRoPE BF16 has slightly larger roundoff at a few elements.
        ATOL, RTOL = (1.5e-2, 2e-2)
    else:
        ATOL, RTOL = (1e-2, 1e-2)
