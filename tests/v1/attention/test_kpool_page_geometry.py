...
def test_sparse_mla_kernel_paged_view_repages_large_blocks():
    ...
    from vllm.v1.kv_cache_interface import MLA_KERNEL_PAGE_ROWS
    ...
    paged = _kernel_paged_view(cache, rows, block_size, block_stride_rows)
    page = MLA_KERNEL_PAGE_ROWS
    ...
    # Kernel-sized blocks are passed through untouched.
    small = torch.zeros(4, 64, head_dim, dtype=torch.uint8)
    assert _kernel_paged_view(small, *flat_kv_row_view(small, 64), 64).shape == (
        4,
        1,
        64,
        head_dim,
...
def test_layout_alignment_rule_matches_kernel_paging():
    ...
    from vllm.v1.kv_cache_interface import MLA_KERNEL_PAGE_ROWS, MLAAttentionSpec

    for block_size, kpool in ((1024, 4), (1152, 4), (256, 4), (64, 1), (4096, 16)):
        ...
        expected = block_size if block_size in (32, 64) else MLA_KERNEL_PAGE_ROWS
        assert _kernel_page_rows(mla) == expected
